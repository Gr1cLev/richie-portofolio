...
export default function AppIcon({
  ...
  isExternal, externalUrl, useGithubIcon, isChat, badge, index,
}) {
  const { state, dispatch } = useOS();
  const longPressTimer = useRef(null);

  const handlePointerDown = (e) => {
    e.stopPropagation(); // prevent Desktop's wallpaper long-press
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <div className="relative">
        ...
        {badge && (
          <div
            className="absolute -top-1 -right-1 min-w-[18px] h-[18px] bg-red-500 rounded-full flex items-center justify-center pointer-events-none"
            style={{ fontSize: '10px', fontWeight: 700, color: '#fff', padding: '0 4px', boxShadow: '0 1px 4px rgba(0,0,0,0.4)' }}
          >
            {badge}
          </div>
        )}
